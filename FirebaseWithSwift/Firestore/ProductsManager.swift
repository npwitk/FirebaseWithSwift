...
//    }
//    
//    private func getAllPorductsForCategory(category: String) async throws -> [Product] {
//        try await productsCollection
...
//    }
    ...
    private func getAllPorductsForCategoryQuery(category: String) -> Query {
        productsCollection
            .whereField(Product.CodingKeys.category.rawValue, isEqualTo: category)
    }
    ...
    func getAllProducts(priceDescending descending: Bool?, forCategory category: String?, count: Int, lastDocument: DocumentSnapshot?) async throws -> (products: [Product], lastDocument: DocumentSnapshot?) {
        ...
        if let descending, let category {
            ...
            query = getAllPorductsForCategoryQuery(category: category)
        }
    ...
    func getProductsByRating(count: Int, lastRating: Double?) async throws -> [Product] {
        try await productsCollection
            ...
            .limit(to: count)
            .start(after: [lastRating ?? 99999999]) // BAD
            .getDocuments(as: Product.self)
    }
